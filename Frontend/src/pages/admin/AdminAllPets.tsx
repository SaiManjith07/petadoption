...
import { PawPrint, Search, Activity, Eye, Calendar, CheckCircle, Stethoscope, FileQuestion, Heart, Smile, AlertCircle } from 'lucide-react';
...
export default function AdminAllPets() {
  ...
  return (
    <AdminLayout onRefresh={loadAllPets} isRefreshing={petsLoading}>
              ...
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                <div>
                  <CardTitle className="text-2xl sm:text-3xl font-bold text-gray-900 flex items-center gap-2">
                    <PawPrint className="h-6 w-6 sm:h-8 sm:w-8 text-[#4CAF50]" />
                    All Pets
                  </CardTitle>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                <Card className="bg-white shadow-md hover:shadow-lg transition-all duration-300 border-l-4 border-l-green-500">
                  <CardHeader className="p-4 pb-2 flex flex-row items-center justify-between space-y-0">
                    <CardTitle className="text-sm font-medium text-gray-600">Found Pets</CardTitle>
                    <CheckCircle className="h-4 w-4 text-green-500" />
                  </CardHeader>
                  <CardContent className="p-4 pt-0">
                    <div className="text-2xl sm:text-3xl font-bold text-gray-900">
                      {pets.filter(p => p.adoption_status === 'Found' || p.adoption_status === 'Listed Found').length}
                    </div>
                  </CardContent>
                </Card>
                <Card className="bg-white shadow-md hover:shadow-lg transition-all duration-300 border-l-4 border-l-red-500">
                  <CardHeader className="p-4 pb-2 flex flex-row items-center justify-between space-y-0">
                    <CardTitle className="text-sm font-medium text-gray-600">Lost Pets</CardTitle>
                    <AlertCircle className="h-4 w-4 text-red-500" />
                  </CardHeader>
                  <CardContent className="p-4 pt-0">
                    <div className="text-2xl sm:text-3xl font-bold text-gray-900">
                      {pets.filter(p => p.adoption_status === 'Lost' || p.adoption_status === 'Listed Lost').length}
                    </div>
                  </CardContent>
                </Card>
                <Card className="bg-white shadow-md hover:shadow-lg transition-all duration-300 border-l-4 border-l-blue-500">
                  <CardHeader className="p-4 pb-2 flex flex-row items-center justify-between space-y-0">
                    <CardTitle className="text-sm font-medium text-gray-600">For Adoption</CardTitle>
                    <Heart className="h-4 w-4 text-blue-500" />
                  </CardHeader>
                  <CardContent className="p-4 pt-0">
                    <div className="text-2xl sm:text-3xl font-bold text-gray-900">
                      {pets.filter(p => p.adoption_status === 'Available for Adoption').length}
                    </div>
                  </CardContent>
                </Card>
                <Card className="bg-white shadow-md hover:shadow-lg transition-all duration-300 border-l-4 border-l-purple-500">
                  <CardHeader className="p-4 pb-2 flex flex-row items-center justify-between space-y-0">
                    <CardTitle className="text-sm font-medium text-gray-600">Reunited/Adopted</CardTitle>
                    <Smile className="h-4 w-4 text-purple-500" />
                  </CardHeader>
                  <CardContent className="p-4 pt-0">
                    <div className="text-2xl sm:text-3xl font-bold text-gray-900">
                      {pets.filter(p => p.adoption_status === 'Reunited' || p.adoption_status === 'Adopted').length}
                    </div>
                  </CardContent>
                </Card>
              </div>
              ...
              <div className="flex flex-col sm:flex-row gap-4 mb-4">
                ...
                <div className="flex flex-wrap gap-2">
                  <select
                    ...
                    className="flex-1 sm:flex-none px-3 py-2 border rounded-md text-sm focus:ring-2 focus:ring-green-500"
                  >
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                        {filtered.map((p: any) => {
                          ...
                          const getBorderColorClass = (status: string) => {
                            ...
                            if (s === 'found' || s.includes('found')) return 'border-l-green-500';
                            if (s === 'lost' || s.includes('lost')) return 'border-l-red-500';
                            if (s === 'adopted' || s.includes('adopted')) return 'border-l-blue-500';
                            if (s.includes('adoption')) return 'border-l-blue-400';
                            return 'border-l-yellow-500'; // Default/Pending
                          };

                          return (
                            <Card key={petId} className={`bg-white rounded-xl shadow-[0_4px_12px_rgba(0,0,0,0.05)] border border-gray-100 hover:shadow-xl transition-all duration-300 transform hover:-translate-y-1 overflow-hidden flex flex-col border-l-4 ${getBorderColorClass(p.adoption_status)}`}>
                              {/* Pet Image */}
