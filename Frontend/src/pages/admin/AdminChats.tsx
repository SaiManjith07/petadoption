    ...
    try {
      ...
      if (approved) {
        ...
        if (result?.verification_room_id) {
          ...
          if (shouldNavigate) {
            navigate(`/admin/chat/${result.verification_room_id}`);
          }
        }
      } else {
          ...
          <CardHeader>
            <div className="flex gap-4 border-b">
              ...
              <button
                ...
                className={`pb-2 px-1 font-medium transition-colors ${activeTab === 'chats'
                  ? 'border-b-2 border-green-600 text-green-600'
                  : 'text-gray-600 hover:text-gray-900'
                  }`}
              >
              ...
              <button
                ...
                className={`pb-2 px-1 font-medium transition-colors ${activeTab === 'closed'
                          ...
                          return (
                            <TableRow key={chat.id || roomId || `chat-${index}`}>
                              ...
                              <TableCell className="text-right">
                                <div className="flex justify-end gap-2">
                                  {(() => {
                                    const currentUserId = user?.id || (user as any)?._id || (user as any)?.user_id;
                                    ...
                                    if (isCreator || isParticipant || isVerifyingAdmin) {
                                      return (
                                        <Button
                                          ...
                                          onClick={() => {
                                            if (roomIdToOpen) {
                                              navigate(`/admin/chat/${roomIdToOpen}`);
                                            } else {
                                              toast({
                                                title: 'Error',
